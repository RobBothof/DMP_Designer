...
public class Shapes_3 : IGenerator {
    ...
    public void Generate(int seed) {
        ...
        for (int lineIndex=0;lineIndex<count;lineIndex++) {
            ...
            for(float r=_rand.NextSingle()*20f;r<360f;r=r+(5f + _rand.NextSingle()*60f)) {
                ...
                if (dotIndex==0) {
                    startpos=Vector2.Transform(new Vector2(0,_rand.NextSingle()*160f+100f),rot) + center;
                    lineDots.Add(startpos);
                } else {
                    lineDots.Add(Vector2.Transform(new Vector2(0,_rand.NextSingle()*160f+100f),rot) + center);
                }
                dotIndex++;
            }
            lineDots.Add(startpos);
