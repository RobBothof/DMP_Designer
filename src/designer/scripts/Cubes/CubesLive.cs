...
public class CubesLive : IGenerator
{
    ...
    public Vector3 SpotLight = new Vector3(1.0f, 0.0f, 0.2f);
    ...
    public void Generate(int seed)
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            Cube(new Vector3(xpos[i] * 33, _rand.NextSingle() * 20 - 10, -750), _rand.NextSingle() * 20 + 10, Quaternion.CreateFromAxisAngle(Vector3.Normalize(new Vector3(_rand.NextSingle(), _rand.NextSingle(), _rand.NextSingle())), (float)Math.PI * 2 * _rand.NextSingle()));
        }
