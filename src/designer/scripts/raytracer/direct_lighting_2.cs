...
public class DirectLighting2 : IGenerator
{
    ...
    public void Generate(int seed, CancellationToken token)
    {
        ...
        for (int x = 0; x < 14; x++)
        {
            for (int y = 0; y < 21; y++)
            {
                _cuboids.Add(Cuboid.Create(new Vector3(x * 2 - 13f + _rng.RandomFloat() * 0.1f, y * 2 - 20 + _rng.RandomFloat() * 0.1f, 0), new Vector3(1f, 1f, 1f), Quaternion.CreateFromAxisAngle(Vector3.Normalize(new Vector3(x * 2 - 13f, y * 2 - 20, 0)), (float)y * MathF.PI * 0.1f), mat1));
            }
        }
