...
public class idea007 : IGenerator {
    public static Random _rand;
    // Vector3 PaperCenter = new Vector3(268800, 380160, 0);
    Vector3 PaperCenter = new Vector3(268800, 330160, 0);
    ...
    public void Generate(int seed) {
        ...
        float step = 4 * MathF.PI * 0.1666666666f;
        for (int i=0; i < 13; i++) {
            ...
            Vector3 P = PaperCenter + Vector3.Transform(new Vector3(0, 150000 + 80000 * MathF.Cos((i*MathF.PI)/2f), 4000), Quaternion.CreateFromAxisAngle(Vector3.UnitZ, (rotationOffset + i * step)));
            Points.Add(P);
        ...
        for (int p=0; p<PointsArray.Length - 1; p++) {
            ...
            Vector3 pt = Vector3.Lerp(PointsArray[p],PointsArray[p+1],0.5f);
            // Vector3 pc = Vector3.Lerp(PaperCenter,pt,(float)_rand.NextDouble()+0.5f);
            Vector3 pc = Vector3.Lerp(PaperCenter,pt,1.35f);
            pc = new Vector3(pc.X,pc.Y,6500);
