...
public class LandShape3vert : IGenerator
{
    ...
    public void Generate(int seed, CancellationToken cancellationToken)
    {
        ...
        // Draw Shadow Lines - VERTICAL VERSION
        float xStep = paper.X / numShadowLines;
        ...
        // Use randomized indices for drawing vertical shadow lines
        Parallel.ForEach(shadowLineIndices, s =>
        {
            _cancellationToken.ThrowIfCancellationRequested();
            // Changed to vertical lines - constant X, varying Y
            Vector3 from = new Vector3(xStep * s, 0, 0);
            Vector3 to = new Vector3(xStep * s, paper.Y, 0);
            Vector3 P0 = from;
            Vector3 P1 = to;

            float ySteps = paper.Y / stepSize;
