...
public class LandShape4c : IGenerator
{
    ...
    public void Generate(int seed, CancellationToken cancellationToken)
    {
        ...
        int[] partialShadowLineIndices = shadowLineIndices.Take((int)(numShadowLines * 0.6f)).ToArray();

        // Use randomized indices for drawing shadow lines
        Parallel.ForEach(partialShadowLineIndices, s =>
        {
            _cancellationToken.ThrowIfCancellationRequested();
