...
public class LandShape4d : IGenerator
{
    ...
    public void Generate(int seed, CancellationToken cancellationToken)
    {
        ...
        ConcurrentBag<Line> shadowLines = new ConcurrentBag<Line>();
        ...
        // Shuffle the lines in Data.lines
        List<Line> shuffledShadowLines = shadowLines.ToList();
        for (int i = shadowLines.Count - 1; i > 0; i--)
        {
            int j = (int)(_rng.RandomFloat() * (i + 1));
            // Swap lines[i] with the element at random index
            Line temp = shuffledShadowLines[i];
            shuffledShadowLines[i] = shuffledShadowLines[j];
            shuffledShadowLines[j] = temp;
        }

        // Add only 60% of the shuffled lines to Data.lines
        for (int i = 0; i < shuffledShadowLines.Count*0.6f; i++)
        {
            Data.lines.Add(shuffledShadowLines[i]);
        }
    }
