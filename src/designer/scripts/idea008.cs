...
public class idea008 : IGenerator {
    ...
    public void Generate(int seed) {
        ...
        for (int i=0; i < 26; i++) {
            ...
            Vector3 P = PaperCenter + Vector3.Transform(new Vector3(0, 130000 + MathF.Sin((i/25f)*10f*MathF.PI) * 80000,3000), Quaternion.CreateFromAxisAngle(Vector3.UnitZ, (rotationOffset + i * step)));
            Points.Add(P);
        }
        // Points.Add(Points[0]);
        // Points.Add(Points[1]);
        ...
        for (int p=0; p<PointsArray.Length - 1; p++) {
            ...
            Vector3 pc = Vector3.Lerp(PaperCenter,pt,2.333f);
            pc = new Vector3(pc.X,pc.Y,6000);
