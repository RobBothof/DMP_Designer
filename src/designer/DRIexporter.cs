...
namespace Designer {
    ...
    public class DriExporter {
        ...
        void AddBezier3DSegment(Int64 startX, Int64 startY, Int64 startZ, Int64 controlX, Int64 controlY, Int64 controlZ, Int64 endX, Int64 endY, Int64 endZ) {
            ...
            if (normalXZ > normalXY && normalXZ > normalYZ) projection = 1; // 3d plane orientation (xz)
            if (normalYZ > normalXY && normalYZ > normalXZ) projection = 2; // 3d plane orientation (zy)
            ...
            if (cur == 0) { // no curve || straight line
                if (projection == 0) AddLine3D(startX, startY, startZ, endX, endY, endZ);
                if (projection == 1) AddLine3D(startX, startZ, startY, endX, endZ, endY);
                if (projection == 2) AddLine3D(startZ, startY, startX, endZ, endY, endX);
            } else {
